...
def create_picture(df_in, target_rows_lb, target_rows_db, name):
    colors = []
    print(target_rows_lb, target_rows_db)
    for row in range(0, df_in.shape[0]):
    ...
    table = ax.table(cellText=df_in.values, cellColours=colors,  loc='center')
...
def create_slide_1(df_6):
    ...
    df_p2.reset_index(inplace=True, drop=True)
    target_rows_lb2 = []
    target_rows_db2 = [1]
    ...
    df_p3.reset_index(inplace=True, drop=True)
    target_rows_lb3 = []
    target_rows_db3 = []
...
def create_slide_2(df_6):
    ...
    df_p2.reset_index(inplace=True, drop=True)
    target_rows_lb2 = [2]
    last = df_p2.index[df_p2[1] == '合計'].tolist()
    target_rows_db2 = [1, last[0]]
    pic_p2 = create_picture(df_p2, target_rows_lb2, target_rows_db2, 's1p2.png')
...
def create_slide_3(df_6):
    ...
    df_p1.reset_index(inplace=True, drop=True)
    last = df_p1.index[df_p1[1] == '合計'].tolist()
    target_rows_lb1 = [last[0]]
    target_rows_db1 = [1, 2]
